 ...
 * @author coder
 */
public class LastnameMenuRule implements ServiceRule{
    
     @Override
    public Boolean matches(String menucode)
    {
        return  menucode.equalsIgnoreCase(new RuleTagsModel().getServiceRule(LastnameMenuRule.class));
    }
    ...
    private Object processRequest(RequestModel request){
        ...
        {
             case "" -> {
                var msg = "Invalid Response. Lastname cannot be empty, kindly try again.";
            ...
            default -> {
                ...
                if (!new RedisDao().cacheOperation(cacheKeyTwo,cacheObject.put("lastname", userResponse)))
                {
                    ...
                    response =  new MenusMananger(request).idNumberMenu();
                }
            }
        }
